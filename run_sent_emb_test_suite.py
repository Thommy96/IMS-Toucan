...
def test_controllable(version, model_id="Meta", exec_device="cpu", speaker_reference=None, vocoder_model_path=None, biggan=False, sent_emb_extractor=None, prompt:str=None, make_preprompt=False, sent_emb_adaptor=None, xvect_model=None, speaker_id_reference=None):
    os.makedirs("audios", exist_ok=True)
    os.makedirs(f"audios/{version}", exist_ok=True)
    ...
    if speaker_reference is not None:
        tts.set_utterance_embedding(speaker_reference)
    if speaker_id_reference is not None:
        tts.set_speaker_embedding(speaker_id_reference)
...
if __name__ == '__main__':
    ...
    if use_speaker_reference:
        speaker_reference = "/mount/resources/speech/corpora/EmoV_DB/anger_449-476_0466-16bit.wav"
        #speaker_reference = "/mount/resources/speech/corpora/LibriTTS/all_clean/1638/84448/1638_84448_000057_000006.wav"
    else:
        speaker_reference = None
    if use_speaker_id:
        speaker_id_reference = "/mount/resources/speech/corpora/EmoV_DB/amused_367-392_0376-16bit.wav"
    else:
        speaker_id_reference = None
    ...
    if use_sent_emb_adaptor:
        ...
        sent_emb_adaptor = SentenceEmbeddingAdaptor(sent_embed_dim=768, utt_embed_dim=64, speaker_embed_dim=512)
        check_dict = torch.load("Models/SentEmbAdaptor_01_EmoMulti_emoBERTcls_xvect/adaptor.pt", map_location=exec_device)
    ...
    #test_controllable(version="ToucanTTS_06_EmoVDB_sent_emb_a14_emoBERTcls_ecapa_rec_loss_ref", model_id="06_EmoVDB_sent_emb_a14_emoBERTcls_ecapa_rec_loss", exec_device=exec_device, vocoder_model_path=None, biggan=True, speaker_reference=speaker_reference, sent_emb_extractor=sent_emb_extractor, prompt=prompt, sent_emb_adaptor=sent_emb_adaptor, xvect_model=xvect_model, speaker_id_reference=speaker_id_reference)

    test_controllable(version="ToucanTTS_02_EmoVDB_ref", model_id="02_EmoVDB", exec_device=exec_device, vocoder_model_path=None, biggan=True, speaker_reference=speaker_reference, xvect_model=xvect_model, speaker_id_reference=speaker_id_reference)
